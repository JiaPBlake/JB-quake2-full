...
//J START New command he's adding in as a demo
...
=================
*/
void ClientCommand (edict_t *ent) //The entity doing the command.  
{
	char	*cmd;
	...
	cmd = gi.argv(0);	//Grabbing from the IMPORTS  the command

	if (Q_stricmp (cmd, "players") == 0)	//checking alphabetical sorting. 0 if same
	{
		Cmd_Players_f (ent);
		return;
	}
